...
::Begin Log
set error="%tmp%\ElectrumTweakerError.txt"
set log="%tmp%\ElectrumTweakerLog.txt"
...
if %errorlevel% neq 0 (
call:ElectrumTweakerLogo
echo              %col2%Run ElectrumTweaker as Admin%col1%
powershell -NoProfile -NonInteractive -Command start -verb runas "'%~s0'" >nul 2>&1
if !errorlevel! equ 0 exit /b
call:ElectrumTweakerLogo
echo.
echo            %col2%ElectrumTweaker is not running as Admin!
...
::Check For Updates
curl -g -k -L -# -o "%tmp%\latestVersion.bat" "https://raw.githubusercontent.com/cataclyysm/ElectrumTweaker/main/Files/Version" >nul 2>&1
call "%tmp%\latestVersion.bat"
if "%DevBuild%" neq "Yes" if "%Version%" lss "!latestVersion!" (cls
	call:ElectrumTweakerLogo
	echo.
	echo             Warning, ElectrumTweaker isn't updated.
...
::WMI Settings
Reg add "HKCU\Software\ElectrumTweaker" /f >nul 2>&1
...
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v CORES') do set CORES=%%a
for /f "tokens=*" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v GPU_NAME') do set GPU_NAME=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v mem') do set mem=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v ChassisTypes') do set ChassisTypes=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Degrees') do set Degrees=%%a
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v osarchitecture') do set osarchitecture=%%a
) >nul 2>&1 else (
...
for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v Restore 2^>nul') do set Restore=%%a
if "%Restore%" equ "" (cls
call:ElectrumTweakerLogo
echo.
echo           Let ElectrumTweaker create a Restore Point
echo              (Used to undo all changes^)
choice /c:NY /n /m "%BS%                  [Y] Yes  [N] No"
Reg add "HKCU\Software\ElectrumTweaker" /v Restore /t REG_DWORD /d "!errorlevel!" /f >nul
)

::Slider
for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v opt 2^>nul') do set /a opt=%%a
...
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Throttling 2^>nul') do set "Throttling=%%a"
if "!Throttling!" equ "" Reg add "HKCU\Software\ElectrumTweaker" /v Throttling /t REG_DWORD /d "0" /f >nul
) else (
for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v PowMax 2^>nul') do set "PowMax=%%a"
if "!PowMax!" equ "" Reg add "HKCU\Software\ElectrumTweaker" /v PowMax /t REG_DWORD /d "1" /f >nul
)

for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v NVCP 2^>nul') do set "NVCP=%%a"
for %%a in (391.35 425.31 441.XX 461.72 456.71 457.30 461.72 461.92 466.11) do (
if not defined NVCP if "%NvidiaDriverVersion%" equ "%%a" Reg add "HKCU\Software\ElectrumTweaker" /v NVCP /t REG_DWORD /d "1" /f >nul
)
...
cls
if "%SettingsPage%"=="1" (
for %%i in (Idle sleepstates PowMax Throttling) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 1/4
...
if %SettingsItem% equ 1 if "%Idle%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v Idle /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v Idle /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%sleepstates%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v sleepstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v sleepstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%PowMax%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v PowMax /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v PowMax /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%Throttling%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v Throttling /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v Throttling /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="2" (
for %%i in (cstates pstates KBoost NVCP) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 2/4
...
if %SettingsItem% equ 1 if "%cstates%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v cstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v cstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%pstates%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v pstates /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v pstates /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%KBoost%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v KBoost /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v KBoost /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%NVCP%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v NVCP /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v NVCP /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="3" (
for %%i in (Res DSCP staticip Mouse) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 3/4
...
if %SettingsItem% equ 1 if "%staticip%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v staticip /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v staticip /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 2 if "%DSCP%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v DSCP /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v DSCP /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%Res%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v Res /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v Res /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%Mouse%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v Mouse /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v Mouse /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 5 goto Settings
if %SettingsItem% lss 5 goto Settings

if "%SettingsPage%"=="4" (
for %%i in (DarkMode Restore DisplayScaling honepow) do set %%i=[91moff & for /f "tokens=3 skip=2" %%a in ('Reg query HKCU\Software\ElectrumTweaker /v %%i 2^>nul') do if "%%a"=="0x1" set %%i=[32mon
echo.
echo                                          %col2%Page 4/4
echo  %col1%[%col2%1%col1%] ElectrumTweaker Light Mode !DarkMode!
echo  [90mSwitch the ElectrumTweaker GUI into light mode
echo.
...
if %SettingsItem% equ 1 if "%DarkMode%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v DarkMode /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v DarkMode /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 1 call :DarkMode "change"
if %SettingsItem% equ 2 if "%Restore%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v Restore /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v Restore /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 3 if "%DisplayScaling%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v DisplayScaling /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v DisplayScaling /t REG_DWORD /d "1" /f >nul)
if %SettingsItem% equ 4 if "%honepow%"=="[32mon" (Reg add "HKCU\Software\ElectrumTweaker" /v honepow /t REG_DWORD /d "0" /f >nul) else (Reg add "HKCU\Software\ElectrumTweaker" /v honepow /t REG_DWORD /d "1" /f >nul)
...
echo                  [32mWindows Optimizations%col1%
...
::Power Plan
set ElectrumTweakerPowName=ElectrumTweaker
...
if "%Idle%" equ "0x1" (
set ElectrumTweakerPowName=%ElectrumTweakerPowName% DIdle
::Disable C-States
...
set ElectrumTweakerPowName=%ElectrumTweakerPowName% dsleep
...
if "%PowMax%" equ "0x1" (
set ElectrumTweakerPowName=%ElectrumTweakerPowName% MAX
...
::Apply
if "%honepow%" neq "0x1" powercfg -changename scheme_current "%ElectrumTweakerPowName%" "For ElectrumTweaker Optimizer %Version% (dsc.gg/ElectrumTweaker) By cataclyysm" >>%log% 2>>%error%
...
echo                 [32mLatency Optimizations%col1%
...
Reg add HKCU\Software\ElectrumTweaker /v Throttling /t REG_DWORD /d 0 /f
...
Reg add HKCU\Software\ElectrumTweaker /v Res /t REG_DWORD /d 0 /f
...
Reg add HKCU\Software\ElectrumTweaker /v KBoost /t REG_DWORD /d 1 /f
Reg add HKCU\Software\ElectrumTweaker /v Res /t REG_DWORD /d 1 /f
Reg add HKCU\Software\ElectrumTweaker /v cstates /t REG_DWORD /d 1 /f
Reg add HKCU\Software\ElectrumTweaker /v pstates /t REG_DWORD /d 1 /f
Reg add HKCU\Software\ElectrumTweaker /v sleepstates /t REG_DWORD /d 1 /f
echo msgbox "Warning^! The performance preset will display your CPU at 100%% usage in the task manager^." >"%tmp%\tmp.vbs"
Reg query HKCU\Software\ElectrumTweaker /v performancemode 2>nul || wscript "%tmp%\tmp.vbs"
Reg add HKCU\Software\ElectrumTweaker /v performancemode /t REG_DWORD /d 1 /f
) >nul
cls
echo.
echo       %col1%Speed up %col2%performance%col1%, %col2%latency%col1%, and %col2%ping%col1%
...
Reg add "HKCU\Software\ElectrumTweaker" /v opt /t REG_DWORD /d %errorlevel% /f >nul
goto Slider
...
:GrabSettings
::Power
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v PowMax') do set PowMax=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Throttling') do set Throttling=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Idle') do set Idle=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v NVCP') do set NVCP=%%a) >nul 2>&1
::Advanced
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Debloat') do set Debloat=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v DisplayScaling') do set DisplayScaling=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v KBoost') do set KBoost=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Restore') do set Restore=%%a) >nul 2>&1
::Optional
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v staticip') do set staticip=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Res') do set Res=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v Mouse') do set Mouse=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v DSCP') do set DSCP=%%a) >nul 2>&1
::Power2
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v cstates') do set cstates=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v pstates') do set pstates=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v honepow') do set honepow=%%a) >nul 2>&1
(for /f "tokens=3 skip=2" %%a in ('Reg query "HKCU\Software\ElectrumTweaker" /v sleepstates') do set sleepstates=%%a) >nul 2>&1
goto:eof
